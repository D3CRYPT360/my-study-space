        ...
        <div className={`mt-10 w-full bg-[#FFFCF8] rounded-[30px] px-4 sm:px-6 md:px-10 py-8 transition-all duration-300 ${selectedGrade ? 'h-auto md:h-[501px]' : 'h-auto md:h-[169px]'}`}>          
          ...
          {selectedGrade && (
            <div className="mt-8 flex flex-col lg:flex-row gap-10">
              <div className="w-full lg:w-3/4 grid grid-cols-1 sm:grid-cols-2 gap-4 md:gap-6">
                {subjects.map(({ name, bgClass, icon }) => (
                  <button
                    key={name}
                    onClick={() => setSelectedSubject(name)}
                    className={`flex items-center gap-3 px-6 py-4 rounded-lg transition-colors w-full h-[73.24px] ${
                  ...
                  >
                    ...
                    <span className={`text-4xl ${selectedSubject === name ? 'text-white' : 'text-[#351E1C]'}`}>{name}</span>
                  </button>
                ))}
              </div>
